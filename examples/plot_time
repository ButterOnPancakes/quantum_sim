...
double execute_circuit(int nb_qubits, int nb_gates) {
    ...
    for(int i = 0; i < nb_gates; i++) {
        ...
        if(type == 8) { //Measure
            add_single_qbit_measure(qc, target, 0);
        }
        else {
            GateType gt = type % 4;
            bool controlled = type / 4;
            int ctrl = rand()%nb_qubits;
            if(controlled && target != ctrl) {
                add_double_qbit_gate(qc, target, ctrl, CNOT);
            }
            else {
                add_single_qbit_gate(qc, target, gt);
            }
        }
    }
    ...
    double time = circuit_execute(qc, state, NULL);
...
int main() {
    ...
    fprintf(graph, "set output 'logs/time_complexity_noçomp.png'\n");
    fprintf(graph, "set title 'Time Complexity (No Multithreading)'\n");
