...
void DrawCircuit(QuantumCircuit *circuit) {
    ...
    int num_qubits = circuit->nb_qbits;
...
void DrawResults(QuantumCircuit *qc, QuantumRegister* qreg) {
    ...
    for (int i = 0; i < num_states; i++) {
        ...
        if (prob > 0.0001) {
            char bin_str[32];
            for(int b=0; b<qc->nb_qbits; b++) {
                 bin_str[qc->nb_qbits - 1 - b] = ((i >> b) & 1) ? '1' : '0';
            }
            bin_str[qc->nb_qbits] = '\0';
...
void HandleInput(QuantumCircuit *qc, QuantumRegister *qreg, ClassicalRegister *creg) {
    if (showResults) return;
...
int main(void) {
    ...
    QuantumCircuit *qc = circuit_create(5);

    while (!WindowShouldClose()) {
        HandleInput(qc, qreg, creg);
