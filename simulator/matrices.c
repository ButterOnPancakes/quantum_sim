...
Matrix *matrix_zero(int rows, int cols) {
    ...
    mat->rows = rows;
    mat->cols = cols;
    mat->data = (float complex **)malloc(rows * sizeof(float complex *));

    #if USE_OMP
    #pragma omp parallel for
    #endif
    for (int i = 0; i < rows; i++) {
        mat->data[i] = (float complex *)calloc(cols, sizeof(float complex));
    }
...
Matrix *matrix_of_array(int rows, int cols, float complex data[rows][cols]) {
    Matrix *mat = malloc(sizeof(Matrix));
    mat->rows = rows;
    mat->cols = cols;
    mat->data = (float complex **)malloc(rows * sizeof(float complex *));
...
float complex matrix_get(Matrix *m, int i, int j) {
...
void matrix_set(Matrix *mat, int i, int j, float complex c) {
...
void matrix_tensor_product_stack(Matrix *A, int rowsB, int colsB, float complex dataB[rowsB][colsB], Matrix **out) {
